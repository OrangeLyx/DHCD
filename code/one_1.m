...
for i=1:m-1
    for j=1:n-1
      U_acc_m(i,j)=(-2/miu-1)*sin(x_f+i*h_1)*sin(y_f+j*h_2);%获得精确解矩阵
    end
end
U_acc_v=reshape(U_acc_m,[(m-1)*(n-1),1]);

%经过计算，边界函数值确实是内点函数值
% gap=norm(U_cal-U_acc_v,'inf');%计算误差向量的无穷范数

V=U_cal_m;

    function s=f(x,y)
       s=h_1*h_1*h_2*h_2*(4+2*a+b)*sin(x)*sin(y)/Q;
    end
